...
class TestPageFollowRequestsAction:
    ...
    def test_follow_to_public_page(self, user_api_client, simple_user, request):
        ...
        response = user_api_client.patch(f"/api/v1/pages/{page.id}/follow/")
        page = Page.objects.first()
        assert response.status_code == 200
        assert page.followers.count() == 1
        assert simple_user.id in page.followers.values_list(flat=True)

    @pytest.mark.usefixtures("owner_user", "simple_user", "private_page")
    def test_follow_to_private_page(self, user_api_client, simple_user, request):
        page = Page.objects.first()
        response = user_api_client.patch(f"/api/v1/pages/{page.id}/follow/")
        page = Page.objects.first()
        ...
        assert simple_user.id in page.follow_requests.values_list(flat=True)
        assert response.status_code == 200
